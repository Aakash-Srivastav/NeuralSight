...
#     image_data = db.Column(db.LargeBinary, nullable=False)  # store binary image directly
...
#         # predictions = Prediction_table.query.filter_by(user_id=user_id).all()
...
#         # Create dictionary with image_name as key
#         patient_data = {}
...
#                     'age': int(row.get('age', 0)) if pd.notna(row.get('age')) and str(row.get('age')).replace('.', '').isdigit() else None,
#                     'symptoms': str(row.get('symptoms', '')) if pd.notna(row.get('symptoms')) else ''
#                 }
#                 patient_data[image_name] = patient_info
#                 print(f"Processed patient data for {image_name}: {patient_info}")
...
#         disclaimer_text = (
#             "This analysis was generated by an AI system and should not be considered as a final medical diagnosis. "
#             "Please consult with a qualified healthcare professional for proper medical evaluation and treatment. "
#             "The AI system's predictions are for informational purposes only and should not replace professional medical advice."
#         )
...
#                 # Get patient data for this image (try exact match first, then partial)
#                 patient_info = patient_data.get(file.filename, {})
                
#                 # If no exact match, try partial matching
#                 if not patient_info:
#                     for key in patient_data.keys():
#                         if key.lower() in file.filename.lower() or file.filename.lower() in key.lower():
#                             patient_info = patient_data[key]
#                             print(f"Found partial match for {file.filename} with {key}")
#                             break
...
# Add a new route to clear patient data
...
# Add a route to check current patient data
